...
def create_full_test_pipeline():
    ...
    # 这里是关键：AnswerNode的默认（成功）出口连接到DistillNode
    answer >> distill
    distill >> evaluation
...
def process_item_and_write_csv(data_item, f_csv, csv_writer, csv_lock):
    ...
    try:
        ...
        result_row = [
            current_id, shared.get("question"), shared.get("distilled_answer", "DISTILL_ERROR"),
...
if __name__ == '__main__':
    ...
    with open(output_csv_filename, 'a', encoding='utf-8-sig', newline='') as f_csv:
        ...
        if f_csv.tell() == 0:
            writer.writerow(['id', 'problem', 'distilled_answer', 'truth', 'final_result'])
