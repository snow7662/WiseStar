...
class AnswerNode(Node):
    def __init__(self, max_retries=1, wait=0, enable_memory=True):
        super().__init__(max_retries, wait)
        ...
        if self.memory_enabled:
            ...
            print(f"✅ [AnswerNode] Memory系统已启用")
        else:
            self.extractor = None
            self.memory = None
...
"""
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        # 保存到Memory系统
        if self.memory_enabled:
            self._save_to_memory(shared)
        
        print(f"📝 [AnswerNode] 🏁 流程完成")
        return None
    
    def _save_to_memory(self, shared):
        """保存解题记录到Memory系统"""
        try:
            print(f"\n💾 [AnswerNode] 正在保存到Memory系统...")
            
            question = shared.get('question', '')
            answer = shared.get('answer', '')
            
            if not question:
                print(f"⚠️ [AnswerNode] 没有题目信息，跳过Memory保存")
                return
            
            # 提取知识点标签
            knowledge_data = self.extractor.extract(question)
            
            # 判断解题是否成功
            solve_success = bool(answer and "抱歉" not in answer and "无法解决" not in answer)
            
            # 统计信息
            actions = shared.get('actions', [])
            calculation_results = shared.get('calculation_results', [])
            
            # 计算步数
            total_steps = len(actions)
            calculate_steps = actions.count('calculate')
            successful_calculations = sum(1 for r in calculation_results if r.get('success', False))
            ...
            record = {
                ...
                'difficulty': knowledge_data.get('difficulty', '未知'),
                'problem_type': knowledge_data.get('category', '数学题'),
            ...
            print(f"✅ [AnswerNode] 已保存到Memory (ID: {record_id})")
            print(f"   知识点: {', '.join(knowledge_data.get('tags', [])[:3])}")
            print(f"   解题状态: {'✅ 成功' if solve_success else '❌ 失败'}")
            
        except Exception as e:
            print(f"⚠️ [AnswerNode] Memory保存失败: {str(e)}")
            print(f"   解题完成，但未记录到学习历史")
